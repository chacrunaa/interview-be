  ...
  async deleteInterview(query: QueryParamsDelete) {
    ...
    if(interviewId){
      if( !isNumber(+interviewId)){
        throw new HttpException('id должно представлять собой число', HttpStatus.BAD_REQUEST);
      }

      const data = await this.interviewsRepository.findOne({where: {id: interviewId}})
      if(!data){
        throw new HttpException('Статьи с данным id не существует', HttpStatus.BAD_REQUEST);
      } else {
        try {
          await this.interviewsRepository.destroy({where: {id: interviewId}})          
          const response = {data: {id:data.id }, message:'Статья успешно удалена'}         
          return response
          
        } catch (error) {
          throw new HttpException('Произошла непредвиденная ошибка', HttpStatus.BAD_REQUEST);
        }
   
      }
    } else {
      throw new HttpException('Должен быть передан id статьи', HttpStatus.BAD_REQUEST);
    }
  }
  ...
  async updateInterview(query:QueryParamsPut){
    ...
    if(interviewId){
      ...
      if(!data){
        ...
        try {
          await this.interviewsRepository.update({...body,  prefix: generatePrefixOfInterview({
            maxoffer: body.maxoffer,
            minoffer: body.minoffer,
          })},{where:{id:interviewId}}) 
          console.log(Interview);

          const newData = await this.interviewsRepository.findOne({where: {id: interviewId}})
