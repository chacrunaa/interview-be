...
export class CreateInterviewDto {
  ...
  @ApiProperty({ example: ['successful'], enum: StatusEnum,isArray:true, description: 'Финальный статус взаимодействия с компанией' })
   readonly status: StatusEnum[];

  @IsArray()
  @ArrayNotEmpty()
  @IsEnum(GradeEnum, { each: true })
   @ApiProperty({ example: ['junior', 'middle', 'senior'], enum: GradeEnum,isArray:true, description: 'Грейд требуемый компанией. Может быть несколько значений' })
   readonly grade: GradeEnum[];
  @IsArray()
  @ArrayNotEmpty()
  @IsEnum(StageEnum, { each: true })
   @ApiProperty({ example: ['livecoding','conversation','techpart'], enum: StageEnum,isArray:true, description: 'Этап взаимодействия с компанией' })
